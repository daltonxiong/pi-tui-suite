...
### 手机端全屏（Android / Termux，Termux 竖屏 48 列）

> 下面这张是**用真实渲染函数生成**的（`node tools/render-readme-screen.mjs`）：头部、消息线框、输入框、
> 底部状态行都走套件里的同一套代码，底部**两排虚拟键**则是照 `~/.termux/termux.properties` 的
> `extra-keys` 布局画的 —— 也就是你在 Termux 里实际看到的那两排。
...
│   deepseek/deepseek-flash · medium effort    │
│            /home/user/my-project             │
╰──────────────────────────────────────────────╯

╭─ TOOL bash ✓ ────────────────────────────────╮
│ $ ls src                                     │
│ api.ts  cache.ts  index.ts                   │
╰──────────────────────────────────────────────╯
╭─ THINK ──────────────────────────────────────╮
│ 现在把重试逻辑独立出来…                      │
╰──────────────────────────────────────────────╯
╭─ ASSISTANT ──────────────────────────────────╮
│ 已将重试逻辑抽成 `withRetry()`，默认 3 次退  │
│ 避重试。                                     │
╰──────────────────────────────────────────────╯
╭─ USER ───────────────────────────────────────╮
│ 顺便给它加个超时参数                         │
...
╰──────────────────────────────────────────────╯
╭─ ASSISTANT ──────────────────────────────────╮
│ 已加上 `timeoutMs` 参数，默认 10s。          │
╰──────────────────────────────────────────────╯
deepseek/deepseek-flash                   ¥42.50
╭ med ───────── ↻ 91.6% ▤━━━━╸───── 45.3%/128k ╮
│ > 帮我加个超时参数                           │
╰────── ↑ 2.1k · ↓ 600 · » 18.4tok/s · ◷ 1m12s ╯
 ↳ 把 src/api.ts 里的重试逻辑抽成函数
┌──────────────────────────────────────────────┐
│        ESC  ☰  HOME  ↑  END  PGUP  ↲         │
│          ↹  CTRL  ←  ↓  →  PGDN  ⌨           │
└──────────────────────────────────────────────┘
...
软键盘展开时终端会从 43 行缩到 25 行（头部固定占 13 行，对话区只剩几行）——
所以窄屏下的元素取舍都是按“哪个在前、哪个先被裁”来排序的。

### 输入框细节：窄屏 vs 宽屏

**窄屏（手机，48 列）**下「模型名 + 余额」会自动上提到输入框上方单独一行，因为 48 列里塞不下
「左 model·thinking + 右 进度条·余额」—— 否则余额会被截掉：
...
| **复制净化** | 选中/复制时自动剥掉线框装饰（`│`、`╭─╮`、`╰─╯`、行尾填充），代码可以直接粘；正文缩进与 ASCII `\|` 不受影响 |
...
| `copyClean.enabled` | `true` | 复制净化开关；想要"连框线一起复制"就设 `false` |
...
| 方式 | 结果 |
| --- | --- |
| 触摸/鼠标选中（`fullscreenCopyOnSelect` 默认开） | 自动复制，**线框已被剥掉**（`copyClean`） |
| `Ctrl+C` | 复制当前选区（同上，已净化） |
| `Ctrl+X` | 复制最后一条助手回复的**原始文本**（不经过线框，天然干净） |
| `/copy` | 同上（内置命令） |
...
node tools/render-readme-screen.mjs         # 生成 README 里那张手机端全屏示例（--write 直接写回）
```
